...
%F013_MEG_SENSOR_LEVEL_DYNAMIC 此处显示有关此函数的摘要
    ...
    for sub_i = 1:27
        ...
        vmPFC_vv_rdm = f082_ROIs_rdm_ctxDist_calc(sub_vv_coord, rotation, vv_geometry.roi_ctx_dist(sub_i, ROI_num, I, :));
        vv_rdm(:, sub_i) = zscore(vmPFC_vv_rdm');

        [M,I] = max(squeeze(cc_geometry.roi_cc_rotation_rr(sub_i, ROI_num, :)));
        rotation = [[cosd(I); sind(I)], [-sind(I); cosd(I)]];
        vmPFC_cc_rdm = f082_ROIs_rdm_ctxDist_calc(cc_coord, rotation, cc_geometry.roi_vtc_features_dist(sub_i, ROI_num, I, :));
